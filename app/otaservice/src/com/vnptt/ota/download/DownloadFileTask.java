package com.vnptt.ota.download;
...
import com.vnptt.ota.common.VnptOtaState;
import com.vnptt.ota.common.VnptOtaUtils;
import com.vnptt.ota.download.DownloadManager.DownloaderListener;
...
//	private long isDownloadedFwBasic = false;
	//@}
	...
	public DownloadFileTask(Context ctx, String folderPath,String urlFile, String fwBasicUrl, Boolean isDownloaded,
			VnptOtaState downloadState, DownloaderListener listener) {
		...
		try {
			...
			if (tempfwBasicFile.exists()) {
				...
				if ((downloadState.getState() == VnptOtaState.STATE_DOWNLOAD_STOP) || 
					...
					previousFileSizeBasic = tempfwBasicFile.length();	//nctmanh: 02112016 - for new ota flow
					...
					totalSizeBasic = tempfwBasicFile.length();	//nctmanh: 02112016 - for new ota flow
				} else {
			...
			if(tempfwBasicFile.exists() && (previousFileSizeBasic > 0) && (responseCode != HttpURLConnection.HTTP_PARTIAL)){
	...
	private void downloadFirmware() {
		try {
			...
			if(tempFile.exists() && (previousFileSize > 0) && (responseCode != HttpURLConnection.HTTP_PARTIAL)){
